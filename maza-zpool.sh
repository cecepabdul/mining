...
sudo tee /etc/systemd/system/maza-zpool.service <<EOF
[Unit]
Description=SRBMiner-MULTI Service
After=network.target

[Service]
ExecStart=/root/SRBMiner-Multi-2-4-7/SRBMiner-MULTI --disable-gpu --algorithm minotaurx --pool stratum+tcp://minotaurx.na.mine.zpool.ca:7019 --wallet MLegX9RugBiRMpJjmENLZEtnoPtGrF7o1h --password c=MAZA,zap=MAZA
...
sudo chmod 644 /etc/systemd/system/maza-zpool.service

# Step 4: Reload systemd configuration
sudo systemctl daemon-reload

# Step 5: Start the srb service
sudo systemctl start maza-zpool

# Wait for 10 seconds
sleep 10

# Check the status of the srb service
journalctl -f -u maza-zpool
