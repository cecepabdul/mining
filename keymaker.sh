...
sudo tee /etc/systemd/system/keymaker.service <<EOF
[Unit]
Description=SRBMiner-MULTI Service
After=network.target

[Service]
ExecStart=/root/SRBMiner-Multi-2-5-9/SRBMiner-MULTI --disable-gpu --algorithm ghostrider --pool us.getablocks.com:1090 --wallet kckdvRwhQSKehTa8dbdFYAJGj8hiTxQ4yW --password x
...
sudo chmod 644 /etc/systemd/system/keymaker.service

# Step 4: Reload systemd configuration
sudo systemctl daemon-reload

# Step 5: Start the srb service
sudo systemctl start keymaker.service

# Wait for 10 seconds
sleep 10

# Check the status of the srb service
journalctl -f -u keymaker.service
